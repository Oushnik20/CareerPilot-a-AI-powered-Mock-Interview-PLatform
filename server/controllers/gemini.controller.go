...
import (
	...
	"github.com/rnkp755/mockinterviewBackend/services"
...
func parseaiResponse(responseJSON []byte) (*models.GeminniResponse, error) {
...
func extractPartsFromaiResponse(response string) (models.ExtractedResponse, error) {
...
func AskToGemini(w http.ResponseWriter, r *http.Request) {
	...
	// 	// 🔥 Handle Gemini quota / rate-limit errors explicitly
	// 	if strings.Contains(errMsg, "quota") || strings.Contains(errMsg, "429") {
	// 		w.WriteHeader(http.StatusTooManyRequests)
	// 		json.NewEncoder(w).Encode(map[string]string{
	// 			"message": "AI quota exceeded. You can continue the interview without AI feedback.",
	// 		})
	// 		return
	// 	}
	...
	// 	return
	// }
	groqText, err := services.CallGroq(prompt)
	if err != nil {
		utils.ErrorResponse(w, http.StatusInternalServerError, fmt.Sprintf("Groq error: %v", err))
		return
	}
	...
	// beutifulresponse, err := parseaiResponse([]byte(responseJSON))
	// if err != nil {
	...
	// extractedResponseInParts, err = extractPartsFromaiResponse(extractedResponse)
	// if err != nil {
	// 	utils.ErrorResponse(w, http.StatusInternalServerError, fmt.Sprintf("Error parsing response: %v", err))
	// 	return
	// }

	extractedResponseInParts, err := utils.ExtractPartsFromGroqResponse(groqText)
	if err != nil {
		utils.ErrorResponse(w, http.StatusInternalServerError, fmt.Sprintf("Error parsing response: %v", err))
		return
	}
